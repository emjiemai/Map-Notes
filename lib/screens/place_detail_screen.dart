...
class PlaceDetailScreen extends StatefulWidget {
  const PlaceDetailScreen({super.key, required this.place, required this.repository});
...
class _PlaceDetailScreenState extends State<PlaceDetailScreen> {
  ...
  Future<void> _postComment() async {
    ...
    try {
      await widget.repository.addComment(placeId: widget.place.id, comment: text);
  ...
  Future<void> _deleteVisit(Visit visit) async {
    ...
    final remaining = await widget.repository.fetchVisitsForPlace(widget.place.id);
    if (!mounted) return;
                    ...
                    children: [
                      if (photos.isNotEmpty)
                        SizedBox(
                          ...
                          child: PageView(children: [
                            for (final url in photos) Image.network(url, fit: BoxFit.cover),
                          ]),
                        )
                      else
                        Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              children: [
                                Chip(
                                  avatar: Icon(style.icon, size: 16, color: Colors.white),
                              ...
                              Padding(
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    Expanded(child: Text(widget.place.address!, style: const TextStyle(color: Colors.grey))),
                                  ],
                                ),
                              ),
                            if (headline != null) ...[
                              ...
                              Row(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  UserAvatar(userId: headline.userId, radius: 20),
                                        ...
                                        RichText(
                                          text: TextSpan(
                                            ...
                                            children: [
                                              ...
                                              TextSpan(
                                                text: headline.authorName ?? 'Unknown rep',
                                                style: const TextStyle(fontWeight: FontWeight.bold),
                                              ),
                                              TextSpan(
                                                text: ' • ${_relativeTime(headline.createdAt)}',
                                  ...
                                  if (headline.userId == widget.repository.currentUserId)
                                    IconButton(
                            ...
                            const Divider(height: 32),
                            Text('Comments (${comments.length})', style: Theme.of(context).textTheme.titleMedium),
                          ],
                        ),
                      ),
                      for (final visit in comments)
                        ListTile(
                          ...
                          trailing: visit.userId == widget.repository.currentUserId
                              ? IconButton(
                                  icon: const Icon(Icons.delete_outline, size: 20),
                                  tooltip: 'Delete your comment',
                                  onPressed: () => _deleteVisit(visit),
                                )
                              : Text(_relativeTime(visit.createdAt), style: Theme.of(context).textTheme.bodySmall),
                        ),
                      if (headline == null && comments.isEmpty)
                        const Padding(
                          padding: EdgeInsets.all(24),
                          child: Center(child: Text('No visits logged here yet.')),
                        ),
